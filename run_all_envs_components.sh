        ...
        RPM|rpm)
          ...
          case "$test_type" in
            ...
            snowflake)
              ...
              pytest -v -s component-test/test_pep_snowflake.py \
                --html="${report_dir}/report-rpm-snowflake-${env}.html" \
                --self-contained-html
              ;;
            lolor)
              echo "▶️ Running RPM lolor tests for env ${env}"
              pytest -v -s component-test/test_pep_lolor.py \
                --html="${report_dir}/report-rpm-lolor-${env}.html" \
                --self-contained-html
              ;;
            pgbouncer)
              echo "▶️ Running RPM pgbouncer tests for env ${env}"
              pytest -v -s component-test/test_pep_pgbouncer.py \
        ...
        DEB|deb)
          ...
          case "$test_type" in
            ...
            snowflake)
              ...
              ;;
            lolor)
              echo "▶️ Running DEB lolor tests for env ${env}"
              pytest -v -s component-test/test_pep_lolor.py \

...
pg_vectorize_version = os.getenv(f"PGEDGE_PG_VECTORIZE_{pg_major_version}_VERSION", "")
...
rhel_pg_vectorize_package = os.getenv("PG_VECTORIZE_PACKAGE", f"pgedge-pg-vectorize_{pg_major_version}")
...
deb_pg_vectorize_package = os.getenv("DEB_PG_VECTORIZE_PACKAGE", f"pgedge-postgresql-{pg_major_version}-pg-vectorize")

# Additional configuration for extension tests
check_extensions = os.getenv("CHECK_EXTENSIONS", "true").lower() == "true"
base_extensions = [ext.strip() for ext in os.getenv("BASE_EXTENSIONS", "vectorize").split(",") if ext.strip()]
...
def get_container_config(container_type):
    ...
    if container_type == "rhel":
        return {
            ...
            "pguser": rhel_pguser,
            "pg_vectorize_package": rhel_pg_vectorize_package,
        }
    else:  # deb
        return {
            "pgbin": deb_pgbin.rstrip('/'),
            "pguser": deb_pguser,
            "pg_vectorize_package": deb_pg_vectorize_package,
        }
...
def test_component_install(container_name, container_type):
    """Step 2: Install pg-vectorize using package_management module"""
    container_name = container_name.strip()
    if not container_name:
        pytest.skip("No container defined in env")

    config = get_container_config(container_type)
    pg_vectorize_package = config["pg_vectorize_package"]
    ...
    print(f"\n--- Installing {pg_vectorize_package} on {container_name} ({container_type}) ---")

    try:
        success, platform, message = package_management.install_package(container, pg_vectorize_package)
        assert success, f"Package installation failed: {message}"
        print(f"✅ {message}")
        print(f"✅ Platform detected: {platform}")
    except Exception as e:
        pytest.fail(f"Failed to install {pg_vectorize_package}: {str(e)}")
...
def test_component_upgrade(container_name, container_type):
    ...
    print(f"\n--- Upgrading {pg_vectorize_package} on {container_name} ({container_type}) ---")
    ...
    try:
        success, platform, message = package_management.upgrade_package(container, pg_vectorize_package)
        if not success:
            if "already" in message.lower() or "newest" in message.lower():
                pytest.skip(f"{pg_vectorize_package} is already at newest version")
        assert success, f"Package upgrade failed: {message}"
        print(f"✅ {message}")
        print(f"✅ Platform detected: {platform}")
    except Exception as e:
        pytest.fail(f"Failed to upgrade {pg_vectorize_package}: {str(e)}")
...
def test_component_package_version(container_name, container_type):
    ...
    if not pg_vectorize_version:
        pytest.skip(f"No PGEDGE_PG_VECTORIZE_{pg_major_version}_VERSION defined in env, skipping version check")
    ...
    print(f"\n--- Verifying {pg_vectorize_package} version on {container_name} ({container_type}) ---")

    try:
        success, platform, installed_version, message = package_management.verify_package_version(
            container, pg_vectorize_package, pg_vectorize_version
        )
        assert success, f"Version verification failed: {message}"
        print(f"✅ {message}")
        print(f"✅ Platform detected: {platform}")
    except Exception as e:
        pytest.fail(f"Failed to verify {pg_vectorize_package} version: {str(e)}")
...
def test_verify_bundled_files(container_name, container_type):
    ...
    config = get_container_config(container_type)
    actual_package = config["pg_vectorize_package"]
...
def test_init_cluster(container_name, container_type):
    """Initialize PostgreSQL cluster with pg-vectorize GUC parameters"""
    container_name = container_name.strip()
    if not container_name:
        pytest.skip("No container defined in env")
...
def test_package_uninstall(container_name, container_type):
    """Uninstall pg-vectorize package using package_management module"""
    if skip_cleanup:
    ...
    print(f"\n--- Uninstalling {pg_vectorize_package} on {container_name} ({container_type}) ---")

    try:
        success, platform, message = package_management.uninstall_package(container, pg_vectorize_package)
        assert success, f"Package uninstallation failed: {message}"
        print(f"✅ {message}")
        print(f"✅ Platform detected: {platform}")
    except Exception as e:
        pytest.fail(f"Failed to uninstall {pg_vectorize_package}: {str(e)}")

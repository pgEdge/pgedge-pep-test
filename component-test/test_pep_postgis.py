...
base_extensions = [ext.strip() for ext in os.getenv("BASE_EXTENSIONS", "postgis").split(",") if ext.strip()]
components = [comp.strip() for comp in os.getenv("COMPONENTS", f"pgedge-postgis35_{pg_major_version}").split(",") if comp.strip()]
...
def get_container_config(container_type):
    ...
    if container_type == "rhel":
        return {
            ...
            "pguser": rhel_pguser,
            "postgis_package": rhel_package,
            "postgis_lib": rhel_postgis_lib,
        }
    else:  # deb
        return {
            "pgbin": deb_pgbin.rstrip('/'),
            "pguser": deb_pguser,
            "postgis_package": deb_package,
            "postgis_lib": deb_postgis_lib,
        }
...
def test_component_install(container_name, container_type):
    ...
    print(f"\n--- Installing {postgis_package} on {container_name} ({container_type}) ---")

    # Use the package_management module to install the package
    try:
        success, platform, message = package_management.install_package(container, postgis_package)
        assert success, f"Package installation failed: {message}"
        print(f"✅ {message}")
        print(f"✅ Platform detected: {platform}")
    except Exception as e:
        pytest.fail(f"Failed to install {postgis_package}: {str(e)}")
...
def test_component_upgrade(container_name, container_type):
    ...
    print(f"\n--- Upgrading {postgis_package} on {container_name} ({container_type}) ---")
    ...
    try:
        success, platform, message = package_management.upgrade_package(container, postgis_package)
        if not success:
            if "already" in message.lower() or "newest" in message.lower():
                pytest.skip(f"{postgis_package} is already at newest version")
        assert success, f"Package upgrade failed: {message}"
        print(f"✅ {message}")
        print(f"✅ Platform detected: {platform}")
    except Exception as e:
        pytest.fail(f"Failed to upgrade {postgis_package}: {str(e)}")
...
def test_component_package_version(container_name, container_type):
    ...
    print(f"\n--- Verifying {postgis_package} version on {container_name} ({container_type}) ---")
    ...
    try:
        ...
        pytest.fail(f"Failed to verify {postgis_package} version: {str(e)}")


@pytest.mark.parametrize("container_name,container_type", all_containers)
def test_verify_binaries_stripped(container_name, container_type):
    """Step 4: Verify that PostGIS libraries are stripped using file_management module"""
    container_name = container_name.strip()
    if not container_name:
        pytest.skip("No container defined in env")
    ...
    # Get container-specific configuration
    config = get_container_config(container_type)
    postgis_lib_dir = config.get("postgis_lib", rhel_postgis_lib if container_type == "rhel" else deb_postgis_lib)
    ...
    print(f"\n--- Verifying PostGIS libraries are stripped on {container_name} ({container_type}) ---")
    print(f"Library directory: {postgis_lib_dir}")
    print(f"Libraries to check: {postgis_stripped_lib}")

    # Use the file_management module to verify specific libraries are stripped
    try:
        success, details, message = file_management.verify_binaries_stripped(
            container=container,
            binary_path=postgis_lib_dir,
            container_name=container_name,
            binary_names=postgis_stripped_lib  # Check specific libraries from env
        )

        # Display results
        print(f"Total libraries checked: {details['total_binaries']}")
        print(f"Stripped libraries: {details['stripped_binaries']}")

        if not success:
            print(f"⚠️ Unstripped libraries found: {len(details['unstripped_binaries'])}")
            for library in details['unstripped_binaries'][:5]:
                print(f"  - {library}")

        assert success, f"Library stripping verification failed: {message}"
        print(f"✅ {message}")
    except Exception as e:
        pytest.fail(f"Failed to verify libraries are stripped: {str(e)}")
...
def test_verify_sbom(container_name, container_type):
    ...
    if container_type == "rhel":
        ...
        exit_code, output = container.exec_run(
            ...
            f"--signature-file postgis-sbom.json.asc "
        ...
        exit_code, output = container.exec_run(
            ...
            f"{_sq_sig_flag} postgis-sbom.json.asc "
            f"postgis-sbom.json'",
            user="root",
        )
        output_str = output.decode().replace('\xa0', ' ')
...
def test_verify_bundled_files(container_name, container_type):
    ...
    config = get_container_config(container_type)
    actual_package = config["postgis_package"]
...
def test_init_cluster(container_name, container_type):
    ...
    # Define GUC parameters for PostGIS testing
    guc_parameters = {
        "wal_level": "logical",
...
def test_package_uninstall(container_name, container_type):
    """Uninstall PostGIS package using package_management module"""
    if skip_cleanup:
    ...
    print(f"\n--- Uninstalling {postgis_package} on {container_name} ({container_type}) ---")

    # Use the package_management module to uninstall the package
    try:
        success, platform, message = package_management.uninstall_package(container, postgis_package)
        assert success, f"Package uninstallation failed: {message}"
        print(f"✅ {message}")
        print(f"✅ Platform detected: {platform}")
    except Exception as e:
        pytest.fail(f"Failed to uninstall {postgis_package}: {str(e)}")

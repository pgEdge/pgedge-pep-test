...
pgbouncer_bundled_files = os.getenv(
    ...
    "/usr/bin/pgbouncer,/usr/share/doc/pgbouncer,/etc/pgbouncer"
).split(",")
...
def test_pgbouncer_install(container_name):
    ...
    exit_code, output = container.exec_run(
        f"{pkg_mgr} pgedge-enterprise-postgres_17",
        user="root"
    )
...
def pgbouncer_cleanup(container_name):
    ...
    # Step 4: Delete user pgbouncer created by automation setup
    print(f"Removing {pgbouncer_user} User  in {container_name}")
    container.exec_run(f"userdel {pgbouncer_user}", user="root")

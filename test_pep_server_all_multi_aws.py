...
rhel_pgbin = os.getenv("PG_BIN_PATH", "/usr/pgsql-18/bin")
deb_pgbin = os.getenv("DEB_PG_BIN_PATH", "/usr/lib/postgresql/18/bin/")
...
def parse_instances():
    ...
    # Parse RHEL instances
    rhel_instances_str = os.getenv("INSTANCES", "")
    rhel_hostnames = [h.strip() for h in rhel_instances_str.split(",") if h.strip()]

    # Parse Debian instances
    debian_instances_str = os.getenv("DEBIAN_INSTANCES", "")
    debian_hostnames = [h.strip() for h in debian_instances_str.split(",") if h.strip()]

    # Parse Ubuntu instances
    ubuntu_instances_str = os.getenv("UBUNTU_INSTANCES", "")
    ubuntu_hostnames = [h.strip() for h in ubuntu_instances_str.split(",") if h.strip()]

    print(f"DEBUG: RHEL INSTANCES = '{rhel_instances_str}'")
    print(f"DEBUG: DEBIAN_INSTANCES = '{debian_instances_str}'")
    print(f"DEBUG: UBUNTU_INSTANCES = '{ubuntu_instances_str}'")

    # Get usernames and key paths
    ...
    # Get platform-specific usernames (optional)
    rhel_usernames = [u.strip() for u in os.getenv("RHEL_USERNAMES", "").split(",") if u.strip()]
    debian_usernames = [u.strip() for u in os.getenv("DEBIAN_USERNAMES", "").split(",") if u.strip()]
    ubuntu_usernames = [u.strip() for u in os.getenv("UBUNTU_USERNAMES", "").split(",") if u.strip()]

    # Get platform-specific key paths (optional)
    rhel_key_paths = [k.strip() for k in os.getenv("RHEL_KEY_PATHS", "").split(",") if k.strip()]
    debian_key_paths = [k.strip() for k in os.getenv("DEBIAN_KEY_PATHS", "").split(",") if k.strip()]
    ubuntu_key_paths = [k.strip() for k in os.getenv("UBUNTU_KEY_PATHS", "").split(",") if k.strip()]

    print(f"DEBUG: Parsed {len(rhel_hostnames)} RHEL hostnames: {rhel_hostnames}")
    print(f"DEBUG: Parsed {len(debian_hostnames)} Debian hostnames: {debian_hostnames}")
    print(f"DEBUG: Parsed {len(ubuntu_hostnames)} Ubuntu hostnames: {ubuntu_hostnames}")

    # If no hostnames found at all, return empty list
    if not rhel_hostnames and not debian_hostnames and not ubuntu_hostnames:
        print("WARNING: No hostnames found in any INSTANCES variables")
        return []

    # Helper function to add instances
    def add_instances(hostnames, platform, default_username, platform_usernames, platform_key_paths):
        if not hostnames:
            return

        # Determine which username list to use
        if platform_usernames:
            usernames_list = platform_usernames
        elif usernames:
            usernames_list = usernames
        else:
            usernames_list = [default_username] * len(hostnames)

        # Determine which key_path list to use
        if platform_key_paths:
            key_paths_list = platform_key_paths
        elif key_paths:
            key_paths_list = key_paths
        else:
            key_paths_list = ["keys/default.pem"] * len(hostnames)

        # Expand lists if only one value provided
        if len(usernames_list) == 1 and len(hostnames) > 1:
            usernames_list = usernames_list * len(hostnames)
            print(f"DEBUG: Using username '{usernames_list[0]}' for all {platform} instances")

        if len(key_paths_list) == 1 and len(hostnames) > 1:
            key_paths_list = key_paths_list * len(hostnames)
            print(f"DEBUG: Using key path '{key_paths_list[0]}' for all {platform} instances")

        # Create instance configs
        for i, hostname in enumerate(hostnames):
            instance = {
                "hostname": hostname,
                "username": usernames_list[i] if i < len(usernames_list) else default_username,
                "key_path": key_paths_list[i] if i < len(key_paths_list) else "keys/default.pem",
                "name": f"{platform.lower()}-{i + 1}",
                "platform": platform.lower()
            }
            instances.append(instance)
            print(
                f"DEBUG: Added {platform} instance: {instance['name']} -> {instance['hostname']} (user: {instance['username']})")

    # Add all instances
    add_instances(rhel_hostnames, "RHEL", "rocky", rhel_usernames, rhel_key_paths)
    add_instances(debian_hostnames, "Debian", "admin", debian_usernames, debian_key_paths)
    add_instances(ubuntu_hostnames, "Ubuntu", "ubuntu", ubuntu_usernames, ubuntu_key_paths)

    print(f"Successfully loaded {len(instances)} instances total")
    return instances
...
def test_install_components(ssh_manager):
    ...
    # Determine platform and get appropriate components
    instance_platform = getattr(ssh_manager, 'platform', None)
    ...
    # Store platform info for later tests
    ssh_manager.platform = platform
    ssh_manager.pgbin = pgbin


def test_verify_component_versions(ssh_manager):
    """Verify component versions"""

    # Get platform info from previous test
    platform = getattr(ssh_manager, 'platform', None)
    ...
    if not platform:
        ...
        components = rhel_components if platform == "rhel" else deb_components
...
def test_init_cluster(ssh_manager):
    ...
    # Get correct pgbin for platform
    pgbin = getattr(ssh_manager, 'pgbin', rhel_pgbin)

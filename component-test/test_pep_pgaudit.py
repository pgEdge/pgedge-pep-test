...
pgaudit_version = os.getenv(f"PGEDGE_PGAUDIT_{pg_major_version}_VERSION", "")
...
rhel_pgaudit_package = os.getenv("PGAUDIT_PACKAGE", f"pgedge-pgaudit_{pg_major_version}")
...
deb_pgaudit_package = os.getenv("DEB_PGAUDIT_PACKAGE", f"pgedge-postgresql-{pg_major_version}-pgaudit")

# Additional configuration for extension tests
# pgaudit must be in shared_preload_libraries, so CHECK_EXTENSIONS defaults to true
check_extensions = os.getenv("CHECK_EXTENSIONS", "true").lower() == "true"
base_extensions = [ext.strip() for ext in os.getenv("BASE_EXTENSIONS", "pgaudit").split(",") if ext.strip()]
...
def test_component_install(container_name, container_type):
    """Step 2: Install pgaudit using package_management module"""
    container_name = container_name.strip()
    if not container_name:
        pytest.skip("No container defined in env")

    config = get_container_config(container_type)
    pgaudit_package = config["pgaudit_package"]
    ...
    print(f"\n--- Installing {pgaudit_package} on {container_name} ({container_type}) ---")

    try:
        success, platform, message = package_management.install_package(container, pgaudit_package)
        assert success, f"Package installation failed: {message}"
        print(f"✅ {message}")
        print(f"✅ Platform detected: {platform}")
    except Exception as e:
        pytest.fail(f"Failed to install {pgaudit_package}: {str(e)}")
...
def test_component_upgrade(container_name, container_type):
    ...
    print(f"\n--- Upgrading {pgaudit_package} on {container_name} ({container_type}) ---")
    ...
    try:
        success, platform, message = package_management.upgrade_package(container, pgaudit_package)
        if not success:
            if "already" in message.lower() or "newest" in message.lower():
                pytest.skip(f"{pgaudit_package} is already at newest version")
        assert success, f"Package upgrade failed: {message}"
        print(f"✅ {message}")
        print(f"✅ Platform detected: {platform}")
    except Exception as e:
        pytest.fail(f"Failed to upgrade {pgaudit_package}: {str(e)}")
...
def test_component_package_version(container_name, container_type):
    ...
    if not pgaudit_version:
        pytest.skip(f"No PGEDGE_PGAUDIT_{pg_major_version}_VERSION defined in env, skipping version check")
    ...
    print(f"\n--- Verifying {pgaudit_package} version on {container_name} ({container_type}) ---")

    try:
        success, platform, installed_version, message = package_management.verify_package_version(
            container, pgaudit_package, pgaudit_version
        )
        assert success, f"Version verification failed: {message}"
        print(f"✅ {message}")
        print(f"✅ Platform detected: {platform}")
    except Exception as e:
        pytest.fail(f"Failed to verify {pgaudit_package} version: {str(e)}")
...
def test_verify_bundled_files(container_name, container_type):
    ...
    config = get_container_config(container_type)
    actual_package = config["pgaudit_package"]
...
def test_init_cluster(container_name, container_type):
    """Initialize PostgreSQL cluster with pgaudit GUC parameters

    pgaudit requires shared_preload_libraries to be set at server start (PGC_POSTMASTER).
    """
    container_name = container_name.strip()
    if not container_name:
        pytest.skip("No container defined in env")
...
def test_package_uninstall(container_name, container_type):
    """Uninstall pgaudit package using package_management module"""
    if skip_cleanup:
    ...
    print(f"\n--- Uninstalling {pgaudit_package} on {container_name} ({container_type}) ---")

    try:
        success, platform, message = package_management.uninstall_package(container, pgaudit_package)
        assert success, f"Package uninstallation failed: {message}"
        print(f"✅ {message}")
        print(f"✅ Platform detected: {platform}")
    except Exception as e:
        pytest.fail(f"Failed to uninstall {pgaudit_package}: {str(e)}")

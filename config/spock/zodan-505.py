...
class SpockClusterManager:
    ...
    def get_spock_nodes(self, dsn: str) -> List[Dict[str, Any]]:
        ...
        if self.verbose:
            ...
            self.info(
                "[QUERY] SELECT n.node_id, n.node_name, n.location, n.country, n.info, i.if_dsn FROM spock.node n JOIN spock.node_interface i ON n.node_id = i.if_nodeid")
    ...
    def verify_node_prerequisites(self, src_node_name: str, src_dsn: str, new_node_name: str, new_node_dsn: str):
        ...
        try:
            ...
            raise Exception(
                f"Exiting add_node: Database {new_db_name} does not exist on new node. Please create it first.")
        ...
        if user_table_count and int(user_table_count.strip()) > 0:
            ...
            raise Exception(
                f"Exiting add_node: Destination database {new_db_name} has the lolor extension installed or remaining lolor user data. The new node should not have the lolor extension installed")
        else:
        ...
        if user_table_count and int(user_table_count.strip()) > 0:
            ...
            raise Exception(
                f"Exiting add_node: Database {new_db_name} on new node has user-created tables. It must be a freshly created database with no user tables (only system and extension tables allowed).")
        else:
        ...
        try:
            ...
            if src_users_result:
                ...
                if missing_users:
                    ...
                    raise Exception(
                        f"Exiting add_node: New node is missing the following users that exist on source node: {missing_users_str}. Please create these users on the new node before adding it to the cluster.")
                else:
            ...
            if "missing users" in str(e):
                ...
                raise Exception(
                    f"Exiting add_node: Failed to verify user compatibility between source and new node: {str(e)}")
    ...
    def trigger_source_sync_and_wait_on_new_node(self, src_node_name: str, src_dsn: str,
        ...
        try:
            ...
            self.format_notice("✗",
                               f"Unable to wait for sync event from {src_node_name} on new node {new_node_name} (error: {str(e)})")
            raise
        ...
        for rec in nodes:
            ...
            if commit_lsn:
                ...
                if current_lsn and target_lsn and current_lsn >= target_lsn:
                    self.notice(
                        f"    - Slot {slot_name} already at or beyond target LSN (current: {current_lsn}, target: {target_lsn})")
                else:
    ...
    def enable_disabled_subscriptions(self, src_node_name: str, src_dsn: str,
        ...
        for rec in nodes:
            ...
            try:
                ...
                if sync_lsn:
                    self.notice(
                        f"    OK: Using stored sync event from origin node {rec['node_name']} (LSN: {sync_lsn})...")
                    ...
                    self.run_psql(new_node_dsn, sql)
                    self.notice(
                        f"    OK: Waiting for sync event from {rec['node_name']} on new node {new_node_name}...")
                else:
    ...
    def check_spock_version_compatibility(self, src_dsn: str, new_node_dsn: str):
        ...
        self.notice(
            f"Version check passed: All nodes running at least Spock version {min_required_version}. Source node {src_version}, new node {new_version}")
    ...
    def health_check(self, src_node_name: str, src_dsn: str, new_node_name: str = None, new_node_dsn: str = None,
                     check_type: str = "pre"):
        """
        ...
        try:
            result = self.run_psql(src_dsn, "SELECT extversion FROM pg_extension WHERE extname = 'spock';", fetch=True,
                                   return_single=True)
            if result:
        ...
        if new_node_dsn:
            try:
                result = self.run_psql(new_node_dsn, "SELECT extversion FROM pg_extension WHERE extname = 'spock';",
                                       fetch=True, return_single=True)
                if result:
            ...
            for node in nodes:
                try:
                    sub_count = self.run_psql(node['dsn'],
                                              ...
                                              fetch=True, return_single=True)
                    self.format_notice("PASS:",
                                       f"Node {node['node_name']} has {sub_count.strip() if sub_count else '0'} active subscriptions")
                    checks_passed += 1
                except Exception as e:
        ...
        if check_type == "pre" and new_node_dsn:
            ...
            try:
                ...
                if user_table_count and int(user_table_count.strip()) == 0:
                    ...
                    self.format_notice("FAIL:",
                                       f"Destination database has the lolor extension installed or remaining lolor user data in the lolor schema.")
                    checks_failed += 1
            except Exception as e:
    ...
    def show_all_nodes(self, cluster_dsn: str):
        ...
        for node in nodes:
            ...
            self.notice(
                f"    [NODE] {node['node_name']}: {node['node_id']} ({node['location']}, {node['country']}) - {dsn_masked}")
...
Examples:
  ...
  python zodan.py health-check --src-node-name n1 --src-dsn "host=localhost dbname=pgedge port=5431 user=pgedge password=pgedge" --new-node-name n3 --new-node-dsn "host=localhost dbname=pgedge port=5433 user=pgedge password=pgedge" --check-type post --verbose
    ...
    health_check_parser = subparsers.add_parser('health-check',
                                                help='Validate cluster health before or after Z0DAN node addition')
    ...
    health_check_parser.add_argument('--check-type', choices=['pre', 'post'], default='pre',
                                     help='Check type: pre (before add_node) or post (after add_node)')

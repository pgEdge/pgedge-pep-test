...
# pgvector is a coupled component: version env var is PG-major-specific.
pgvector_version = os.getenv(f"PGEDGE_PGVECTOR_{pg_major_version}_VERSION", "0.8.5")

# pgvector ships a PostgreSQL extension library (vector.so); it does not ship a
# standalone /usr/bin binary, so the binary version/strip tests are omitted.
...
rhel_pgvector_package = os.getenv("PGVECTOR_PACKAGE", f"pgedge-pgvector_{pg_major_version}")
# Server package this coupled component depends on (installed alongside pgvector).
rhel_server_dependency = os.getenv("SERVER_PACKAGE", f"pgedge-postgresql{pg_major_version}-server").split(",")[0]
rhel_bundled_files = os.getenv(
    "PGVECTOR_BUNDLED_FILES",
    ""
).split(",")
...
deb_pgvector_package = os.getenv("DEB_PGVECTOR_PACKAGE", f"pgedge-postgresql-{pg_major_version}-pgvector")
deb_server_dependency = os.getenv("DEB_SERVER_PACKAGE", f"pgedge-postgresql-{pg_major_version}").split(",")[0]
deb_bundled_files = os.getenv(
    "DEB_PGVECTOR_BUNDLED_FILES",
    ""
).split(",")

# Additional configuration for extension tests
check_extensions = os.getenv("CHECK_EXTENSIONS", "true").lower() == "true"
base_extensions = [ext.strip() for ext in os.getenv("PGVECTOR_EXTENSIONS", "vector").split(",") if ext.strip()]
components = [comp.strip() for comp in os.getenv("COMPONENTS", f"pgedge-pgvector_{pg_major_version}").split(",") if comp.strip()]
...
def get_container_config(container_type):
    ...
    if container_type == "rhel":
        return {
            ...
            "pguser": rhel_pguser,
            "pgvector_package": rhel_pgvector_package,
            "server_dependency": rhel_server_dependency,
        ...
        return {
            ...
            "pguser": deb_pguser,
            "pgvector_package": deb_pgvector_package,
            "server_dependency": deb_server_dependency,
...
def test_component_install(container_name, container_type):
    """Step 2: Install pgedge-pgvector using package_management module"""
    container_name = container_name.strip()
    if not container_name:
        pytest.skip("No container defined in env")
    ...
    pgvector_package = config["pgvector_package"]
    ...
    print(f"\n--- Installing {pgvector_package} on {container_name} ({container_type}) ---")

    # Use the package_management module to install the package
    try:
        success, platform, message = package_management.install_package(container, pgvector_package)
        assert success, f"Package installation failed: {message}"
        print(f"✅ {message}")
        print(f"✅ Platform detected: {platform}")
    except Exception as e:
        pytest.fail(f"Failed to install {pgvector_package}: {str(e)}")
...
def test_component_upgrade(container_name, container_type):
    ...
    print(f"\n--- Upgrading {pgvector_package} on {container_name} ({container_type}) ---")
    ...
    try:
        success, platform, message = package_management.upgrade_package(container, pgvector_package)
        if not success:
            if "already" in message.lower() or "newest" in message.lower():
                pytest.skip(f"{pgvector_package} is already at newest version")
        assert success, f"Package upgrade failed: {message}"
        print(f"✅ {message}")
        print(f"✅ Platform detected: {platform}")
    except Exception as e:
        pytest.fail(f"Failed to upgrade {pgvector_package}: {str(e)}")
...
def test_component_package_version(container_name, container_type):
    ...
    if not pgvector_version:
        pytest.skip("No PGVECTOR_VERSION defined in env, skipping version check")
    ...
    print(f"\n--- Verifying {pgvector_package} version on {container_name} ({container_type}) ---")
    ...
    try:
        success, platform, installed_version, message = package_management.verify_package_version(
            container, pgvector_package, pgvector_version
        )
        assert success, f"Version verification failed: {message}"
        print(f"✅ {message}")
        print(f"✅ Platform detected: {platform}")
    except Exception as e:
        pytest.fail(f"Failed to verify {pgvector_package} version: {str(e)}")
...
def test_verify_bundled_files(container_name, container_type):
    ...
    config = get_container_config(container_type)
    actual_package = config["pgvector_package"]
...
@pytest.mark.parametrize("container_name,container_type", all_containers)
def test_verify_package_dependencies(container_name, container_type):
    """Verify pgvector declares a dependency on the coupled PostgreSQL server package.

    pgvector is a coupled component: installing it pulls in the matching
    pgedge PostgreSQL server for the PG major version. This checks the
    package metadata declares that dependency.
    """
    container_name = container_name.strip()
    if not container_name:
        pytest.skip("No container defined in env")
    ...
    print(f"\n--- Verifying {pgvector_package} depends on the PostgreSQL server on {container_name} ({container_type}) ---")

    if container_type == "rhel":
        # rpm -qR lists dependencies; the coupled server package appears there.
        cmd = f"rpm -qR {pgvector_package}"
        needle = f"pgedge-postgresql{pg_major_version}"
    else:  # deb
        cmd = f"dpkg -s {pgvector_package}"
        needle = f"pgedge-postgresql-{pg_major_version}"

    exit_code, output = container.exec_run(["bash", "-c", cmd], user="root")
    assert exit_code == 0, f"'{cmd}' failed: {output.decode().strip()}"

    deps_output = output.decode()
    if container_type == "deb":
        # Narrow to the Depends: line for a clearer assertion.
        depends_lines = [ln for ln in deps_output.splitlines() if ln.strip().startswith("Depends")]
        deps_output = "\n".join(depends_lines) if depends_lines else deps_output

    print(f"   Dependencies:\n{deps_output.strip()}")

    assert needle in deps_output, (
        f"Expected dependency containing '{needle}' not found for {pgvector_package}.\n"
        f"Output:\n{deps_output.strip()}"
    )
    print(f"✅ {pgvector_package} declares a dependency on '{needle}'")


@pytest.mark.parametrize("container_name,container_type", all_containers)
def test_verify_license_file(container_name, container_type):
    """Verify the LICENSE file is installed by the package."""
    container_name = container_name.strip()
    if not container_name:
        pytest.skip("No container defined in env")
    ...
    if container_type == "rhel":
        license_path = f"/usr/share/licenses/{pgvector_package}/LICENSE"
    else:  # deb
        license_path = f"/usr/share/doc/{pgvector_package}/copyright"

    print(f"\n--- Verifying license file {license_path} on {container_name} ({container_type}) ---")

    exit_code, output = container.exec_run(["bash", "-c", f"test -f {license_path}"], user="root")
...
def test_verify_readme_file(container_name, container_type):
    """Verify the README file is installed by the package."""
    container_name = container_name.strip()
    if not container_name:
        pytest.skip("No container defined in env")
    ...
    # RHEL ships /usr/share/doc/<pkg>/README.md; Debian gzips it to README.md.gz.
    if container_type == "rhel":
        readme_path = f"/usr/share/doc/{pgvector_package}/README.md"
    else:  # deb
        readme_path = f"/usr/share/doc/{pgvector_package}/README.md.gz"

    print(f"\n--- Verifying README file {readme_path} on {container_name} ({container_type}) ---")

    exit_code, output = container.exec_run(["bash", "-c", f"test -f {readme_path}"], user="root")
...
def test_verify_sbom(container_name, container_type):
    ...
    if container_type == "rhel":
        ...
        exit_code, output = container.exec_run(
            ...
            f"--signature-file pgvector-sbom.json.asc "
            f"--signer-file pgedge-rsa.pub "
            f"pgvector-sbom.json'",
            user="root",
        )
        ...
        exit_code, output = container.exec_run(
            ...
            f"{_sq_sig_flag} pgvector-sbom.json.asc "
            f"pgvector-sbom.json'",
            user="root",
        )
        output_str = output.decode().replace('\xa0', ' ')
...
def test_init_cluster(container_name, container_type):
    ...
    pgvector requires no special shared_preload_libraries or GUC parameters;
    the extension is created on demand via CREATE EXTENSION vector.
    """
    container_name = container_name.strip()
    if not container_name:
        pytest.skip("No container defined in env")
    ...
    # pgvector needs no special GUC parameters.
    guc_parameters = {}
...
def test_extension_version(container_name, container_type, extension):
    ...
    reported version column matches pgvector_version.
    ...
    assert pgvector_version in installed_version, (
        f"Extension '{extension}' version mismatch: "
        f"expected '{pgvector_version}', got '{installed_version}'"
    )
    print(f"✅ Extension '{extension}' version {installed_version} matches expected {pgvector_version}")
...
def test_package_uninstall(container_name, container_type):
    """Uninstall pgvector package using package_management module"""
    if skip_cleanup:
    ...
    print(f"\n--- Uninstalling {pgvector_package} on {container_name} ({container_type}) ---")

    # Use the package_management module to uninstall the package
    try:
        success, platform, message = package_management.uninstall_package(container, pgvector_package)
        assert success, f"Package uninstallation failed: {message}"
        print(f"✅ {message}")
        print(f"✅ Platform detected: {platform}")
    except Exception as e:
        pytest.fail(f"Failed to uninstall {pgvector_package}: {str(e)}")

#!/usr/bin/env python3
"""
Spock Large-Transaction Stress Test (interactive)
==================================================
...
block. This stresses Spock's reorder buffer, the change-record stream,
and the apply worker's throughput.
...
Chaos hooks (--chaos, comma-separated):
  ...
  restart-subscriber : sub_disable + sub_enable mid-replay
  kill-apply-worker  : pg_terminate_backend on spock workers
...
Usage:
  python test_spock_large_txn.py --config nodes2.json
  python test_spock_large_txn.py --config nodes2.json --non-interactive \\
      --rows 50m --row-bytes 50
  python test_spock_large_txn.py --config nodes2.json --cleanup
        ...
        try:
            with self.parent._conn(self.parent.n2) as c, c.cursor(
                ...
                try:
                    ...
                    try:
                        # PostgreSQL / Spock versions expose different
                        # columns in pg_stat_wal_receiver.
                        #
                        # Spock 5.x environments typically do NOT have:
                        #   received_lsn
                        #
...
class ChaosEngine:
    ...
    def _restart_subscriber(self):
        ...
        with self.parent._conn(self.parent.n2) as c, c.cursor() as cur:
            for sn in subs:
                cur.execute(
                    "SELECT spock.sub_disable(%s, immediate := true);",
                    (sn,))
        time.sleep(3)
        with self.parent._conn(self.parent.n2) as c, c.cursor() as cur:
            for sn in subs:
                cur.execute("SELECT spock.sub_enable(%s);", (sn,))
...
class LargeTxnTester:
    ...
    def _get_subs(self, node_name: str) -> List[Dict[str, Any]]:
        """Return subscriptions on a node, normalized across Spock 5.x and
        6.x. Each dict has keys 'name' (str|None) and 'enabled' (bool|None).
        Also includes '_raw' with all original columns.

        Schema differences:
          Spock 5.x: sub_name, sub_enabled, sub_origin_if, sub_target_if, ...
          Spock 6.x: slot_name, status, ... (column names may vary)

        Note: in 6.x the subscription name passed to sub_enable/sub_disable
        is NOT necessarily the slot name. If we can find a more specific
        column ('sub_name', 'subscription_name', 'name'), we prefer that.
        Otherwise we fall back to slot_name and hope it's accepted (in
        most Spock builds it is, since slot_name == "spk_<db>_<node>_<sub>"
        and sub_enable accepts either form).
        """
        try:
            with self._conn(node_name) as c, c.cursor(
                    cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                cur.execute("SELECT * FROM spock.sub_show_status();")
                raw = cur.fetchall()
        except Exception:
            raise
        ...
        for row in raw:
            d = dict(row)
            # name resolution: prefer a 'real' sub name; slot_name only
            # as last resort
            name = (d.get('sub_name')
                    or d.get('subscription_name')
                    or d.get('name')
                    or d.get('slot_name'))
            # enabled resolution
            enabled: Optional[bool] = None
            if 'sub_enabled' in d:
                enabled = bool(d['sub_enabled'])
            elif 'enabled' in d:
    ...
    def preflight(self, plan: PhasePlan) -> bool:
        ...
        for n in (self.n1, self.n2):
            try:
                with self._conn(n) as c, c.cursor() as cur:
                    ...
                    r = cur.fetchone()
                    if not r:
                        self._log(f"[{n}] Spock NOT installed",
                                  "ERROR")
                        ok = False
                    else:
                        self._log(f"[{n}] Spock {r[0]} installed", "OK")
            except Exception as e:
        ...
        try:
            ...
            if not subs:
                ...
                if subs[0].get('_raw'):
                    self._log(
                        f"[{self.n2}] sub_show_status columns: "
    ...
    def _setup_schema(self):
        ...
        """
        for n in (self.n1, self.n2):
            # Use separate connections per statement so a failed
            # repset_remove_table (e.g. table not in repset) can't
            # leave the next DROP in "txn aborted" state.
            try:
                with self._conn(n, autocommit=True) as c, \
                        c.cursor() as cur:
                    try:
                        cur.execute(
                            "SELECT spock.repset_remove_table(%s, %s);",
                            (self.repset, TABLE))
                    except Exception:
                        pass
            except Exception:
        ...
        self._log(f"[{self.n1}] created {TABLE}", "OK")
        time.sleep(2)
        with self._conn(self.n2) as c, c.cursor() as cur:
            cur.execute(
                "SELECT 1 FROM pg_class WHERE relname=%s;", (TABLE,))
            if not cur.fetchone():
                self._log(f"[{self.n2}] DDL didn't replicate; creating "
                          f"manually", "WARN")
                cur.execute(ddl_create)
            else:
                self._log(f"[{self.n2}] {TABLE} present", "OK")
        with self._conn(self.n1) as c, c.cursor() as cur:
            try:
                cur.execute(
                    "SELECT spock.repset_add_table(%s, %s);",
                    (self.repset, TABLE))
                self._log(f"[{self.n1}] added {TABLE} to repset "
                          f"'{self.repset}'", "OK")
            except Exception as e:
                if 'already' in str(e).lower():
                    self._log(f"[{self.n1}] already in repset", "INFO")
                else:
                    raise
    ...
    def _summarize(self):
        ...
        self._log(f"  spock pids seen on n2  : "
...
def main():
    p = argparse.ArgumentParser(
        description="Spock 2-node large-transaction stress test "
                    "(interactive)",
...
Examples:
  ...
  python test_spock_large_txn.py --config nodes2.json --size 2.5GB
